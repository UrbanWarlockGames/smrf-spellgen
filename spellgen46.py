...
def generate_spell(targets, verbs, adverbs, adjectives, magnitude, nounsall, elements, blasttypes, blasttypesof):
    ...
    if random_choice < 0.5:  # 50% chance for this category
        ...
        if sub_random_choice < 0.05:  # 5% chance for magnitude + verb + targets
            ...
            random_word = f"{random.choice(adverbs).capitalize()} {random.choice(nounsall)}"
        else:  # 30% chance for adjectives + targets
            random_word = f"{random.choice(adjectives).capitalize()} {random.choice(nounsall)}"
    else:  # 50% chance for this category
        sub_random_choice = random.random()  # Another random choice within this category

        if sub_random_choice < 0.125:  # 12.5% chance for element + blasttype
            random_word = f"{random.choice(elements)} {random.choice(blasttypes)}"
        elif sub_random_choice < 0.25:  # 12.5% chance for blasttype-of + element
            random_word = f"{random.choice(blasttypesof)} {random.choice(elements)}"
...
def generate_spells(num_results):
    ...
    blasttypesof_file_path = os.path.join(script_dir, 'blasttypesof.txt')
    nounsall_file_path = os.path.join(script_dir, 'nounsall.txt')
    ...
    blasttypesof = read_words_from_file(blasttypesof_file_path)
    nounsall = read_words_from_file(nounsall_file_path)

    spells = []
    for _ in range(num_results):
        spell = generate_spell(targets, verbs, adverbs, adjectives, magnitude, nounsall, elements, blasttypes, blasttypesof)
        spells.append(spell)
    return spells
